...
namespace ContestProject
{
    ...
    public class Code : CodeBase
    {
        ...
        public override string Execute(string input)
        {
            ...
            for (int count = 0; count < countPaths; count++)
            {
                var secondLine = lines[0].Split(' ').Select(int.Parse).ToArray();
                (int x, int y) lawnSize = (secondLine[0], secondLine[1]);
                (int x, int y) treeLocation = getTreeLocation(lines[1..(lawnSize.y + 1)]);
                var path = lines[lawnSize.y + 1];

                results.Add(isValid(lawnSize, treeLocation, path) ? "VALID" : "INVALID");
